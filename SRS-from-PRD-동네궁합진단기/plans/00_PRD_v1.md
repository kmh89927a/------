# 내 하루 동선 맞춤 동네 궁합 진단기 — PRD v0.1-rev.2
...
### Story 3-5: 반복 이사 입력값 저장·재탐색

> **As a** 반복 이사자 (C-04 이수현),
> **I want** 이번 발령지 탐색 결과를 자동 저장하고, 2년 후 새 발령 시 원클릭으로 재탐색 + 미래 시나리오 비교,
> **so that** 매번 원점 재시작(3주) 대신 3일 만에 탐색을 완료할 수 있다.

| **AC** | **Given** | **When** | **Then** | **측정 임계치** |
| --- | --- | --- | --- | --- |
| AC-1 | 사용자가 진단을 완료한 상태 | 세션 종료 또는 앱 종료 | 입력 조건(주소·필터·시간대)이 자동 저장되어 다음 방문 시 복원 가능 | **저장 성공률 ≥ 99.9%**, 복원 시간 ≤ 1초 |
| AC-2 | 이전 탐색 기록이 저장된 상태 (2년 전) | "이전 조건으로 재탐색" 클릭 | 현재 시점의 매물·교통 데이터로 재계산된 결과가 과거 결과와 비교 뷰로 표시 | **재탐색 응답 ≤ 5초**, 비교 뷰 항목 ≥ 5개 |
| AC-3 | 재탐색 결과가 표시된 상태 | 발령 후보지(2~3곳) 추가 입력 | 시나리오별 동선 변화 비교 화면이 생성된다 | **시나리오 최대 3개 동시 비교**, 응답 ≤ 3초 |
| AC-N1 | 2년 전 저장된 주소가 **행정 구역 변경**으로 무효 | "이전 조건으로 재탐색" 클릭 | "저장된 주소 '○○동'이 변경되었습니다" 안내 + 변경된 행정동 자동 매핑 제안 또는 수동 수정 유도 | **행정동 변경 감지 정확도 ≥ 95%** (법정동 코드 매핑 테이블 분기 갱신) |
...
| **S** | 이전 입력값 자동 저장 + 원클릭 재탐색 + 발령 알림 | 🟠 v1 | 3.60 | 2.64 | 반복 이사 패턴 인식 서비스 시장 無. LTV·구독 모델 핵심 근거 |
...
> **내 하루 동선 맞춤 동네 궁합 진단기** | PRD v0.1-rev.2 | 2026-04-12
>
