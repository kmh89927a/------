...
> **문서 버전:** v1.1 | **작성일:** 2026-04-18
> **근거 문서:** SRS-001 Rev 1.5 (2026-04-18), PRD v0.1-rev.3
...
> - §3.1 STK-04 표: "과거 vs 현재 비교 뷰", "시나리오 비교 (3개)" 행 제거 (SRS Rev 1.5 F5 축소 반영)
> - §5 Gantt 차트: Phase 6 블록 5일 → 2일 축소
...
| 검증 항목 | PRD 약속 | 기존 구현 | C-TEC 구현 | 가치 훼손 여부 |
|---|---|---|---|---|
| 입력값 자동 저장 | best effort, 복원 ≤ 1초 | REST API → PostgreSQL | **Server Action → Prisma** (동일 패턴) | ✅ **보존** |
| 불러오기 → 폼 자동 채움 | geocoding 실패 시 "다시 입력" 안내 | REST API | **Server Action** `getSavedSearch()` | ✅ **보존** |

> **결론: "반복 이사 경험 개선" 가치 보존.** 비교 뷰·시나리오 비교는 SRS Rev 1.5에서 제거되어 검증 대상 외.
...
gantt
    ...
    section Phase 6: F5 간이 저장
    saveSearch+불러오기       :p6a, after p5b, 2d

    section Phase 7: 결제+AI
    토스페이먼츠 연동       :p7a, after p6a, 3d
    Vercel AI SDK           :p7b, after p6a, 4d
