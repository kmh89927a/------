...
| API-05 | `saveSearch()` | 입력값 저장 | Server Action | NextAuth Session |
| API-06 | `replaySearch()` | 과거 조건 재탐색 | Server Action | NextAuth Session |
...
| --- | --- | --- | --- | --- |
| **REQ-NF-035** | 에러 로그 알림 | 5xx 에러 5분간 ≥ 10건 → 슬랙 알림 | Sentry (Vercel 통합) | PRD §5-4 |
| **REQ-NF-036** | 응답 시간 경고 | p95 > 목표치 120% → 슬랙 경고 | Vercel Analytics + Sentry Performance | PRD §5-4 |
| **REQ-NF-037** | API 호출량/비용 경고 | 일일 호출량 > 예산 80% → 슬랙 경고 | Vercel Dashboard + Supabase Dashboard | PRD §5-4 |
...
| API-05 | POST | `saveSearch()` | 입력값 저장 | `user_id`: uuid, `search_params`: object | `saved_search_id`: uuid, `saved_at`: timestamp | Server Action | NextAuth Session | ≤ 500ms |
| API-06 | POST | `replaySearch()` | 과거 조건 재탐색 | `saved_search_id`: uuid | `current_result`: DiagnosisResult, `previous_result`: DiagnosisResult, `diff`: ComparisonView | Server Action | NextAuth Session | ≤ 5,000ms |
...
sequenceDiagram
    ...
    alt 링크 만료 (30일 초과)
        ...
        SSR->>Prisma: ViewLog create + view_count 증가
...
sequenceDiagram
    ...
    participant SA as Server Action
    participant Prisma as Prisma ORM
    participant DongMap as 행정동 코드 매핑 (Prisma seed 데이터)

    %% 자동 저장
    User->>Web: 진단 완료 후 세션 종료
    Web->>SA: saveSearch(searchParams)
    SA->>Prisma: SavedSearch create (성공률 ≥ 99.9%)
    SA-->>Web: 저장 확인
    ...
    Web->>SA: replaySearch(savedSearchId)
    SA->>Prisma: SavedSearch 조회

    SA->>DongMap: 저장된 행정동 코드 유효성 검증

    alt 행정 구역 변경됨
        DongMap-->>SA: 변경 감지 (정확도 ≥ 95%)
        SA-->>Web: {changed_address: "○○동", suggestion: "△△동"}
        Web->>User: "저장된 주소 '○○동'이 변경되었습니다" + 자동 매핑 제안
        User->>Web: 매핑 수락 또는 수동 수정
        Web->>SA: replaySearch(수정된 조건)
    else 행정 구역 유효
        SA->>SA: 현재 시점 매물·교통 데이터로 재계산
    end

    SA->>Prisma: 이전 Diagnosis 결과 조회
    SA->>SA: 비교 뷰 생성 (항목 ≥ 5개)
    SA-->>Web: {current_result, previous_result, diff} (≤ 5초)
    Web->>User: 과거 vs 현재 비교 뷰 표시

    %% 시나리오 비교
    User->>Web: 발령 후보지 2~3곳 추가 입력
    Web->>SA: createDiagnosis(scenario_mode, 후보지 리스트)
    SA->>SA: 시나리오별 동선 변화 계산 (최대 3개 동시)
    SA-->>Web: 시나리오 비교 결과 (≤ 3초)
